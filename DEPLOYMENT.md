...
3. Paste the Apps Script example from `README.md`.
4. Add a script property named `LEADS_WEBHOOK_SECRET`.
...
If the form shows a save error, check the Vercel function logs first. The most common causes are a missing `LEADS_WEBHOOK_URL`, a mismatched `LEADS_WEBHOOK_SECRET`, or an Apps Script deployment that is not accessible to `Anyone`.
